...
@section('admin-content')
    ...
    <div class="container mx-auto p-5">
        <h1 class="font-semibold text-lg uppercase">Kelola Template Surat</h1>
        <div class="bg-blue-600 h-[4px] rounded w-28 mt-2 mb-4"></div>

        <div class="text-end my-4">
            @if (!$letters->where('type', 'Template Pernyataan')->count())
                <a href="{{ route('admin.letter.create') }}"
                    class="px-5 py-2 rounded-xl text-white font-semibold bg-indigo-500 hover:bg-indigo-600">+
                    Template Surat Pernyataan</a>
            @endif
            @if (!$letters->where('type', 'Template SPH')->count())
                <a href="{{ route('admin.letter.create') }}"
                    class="px-5 py-2 rounded-xl text-white font-semibold bg-indigo-500 hover:bg-indigo-600">+
                    Template SPH</a>
            @endif
                ...
                <table class="min-w-full text-sm text-left text-gray-500">
                    <thead class="text-xs text-white uppercase bg-indigo-400">
                        <tr>
                            ...
                            <th class="px-6 py-3">template surat</th>
                    ...
                    <tbody>
                        @foreach ($letters as $letter)
                            <tr class="bg-white border-b hover:bg-gray-100">
                                ...
                                <td class="px-3 py-2">{{ $letter->type }}</td>
                                <td class="px-3 py-2 text-center">
                                    <div class="flex justify-center space-x-2">
                                        <a href="{{ asset('storage/' . $letter->file_letter) }}" target="_blank"
                                            class="text-blue-600 hover:underline">Detail</a>
                                        <a href="{{ route('admin.letter.edit', $letter->type) }}"

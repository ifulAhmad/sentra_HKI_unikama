            ...
            <div class="px-12 mb-4 flex items-center gap-3 justify-between">
                ...
                <div class="flex items-center gap-3">
                    @if ($unreadNotifications->count())
                        <form action="{{ route('admin.notifications.markAsReadAll') }}" method="post">
                            @csrf
                    ...
                    @if ($notifications->count())
                        <form action="{{ route('admin.notifications.deleteAllNotif') }}" method="post">
                            @csrf
                            @method('DELETE')
                        ...
                        @foreach ($unreadNotifications as $notif)
                            <a href="{{ $notif->data['link'] }}">
                                <div
                                    ...
                                    <div>
                                        <h1 class="font-semibold">{{ $notif->data['title'] }}</h1>
                                        <p class="font-light ps-2">{!! $notif->data['body'] !!}</p>
                                    </div>
                                    <div class="text-center">
                                        <i class="bi bi-exclamation-circle"></i>
                                        <p class="text-xs font-light">{{ $notif->created_at->diffForHumans() }}</p>
                                    </div>
                                </div>
                            </a>
                        @endforeach
